...
public class SanctionLogin extends AppCompatActivity {
    ...
    public void login(final View v) {
        final String sanction_id = san_name.getText().toString();
        ...
        JsonObjectRequest jobjreq = new JsonObjectRequest("http://"+Ipaddress.ip+"/santion.php", js, new Response.Listener<JSONObject>() {
            @Override
            public void onResponse(JSONObject response) {
                ...
                try {
                    if (response.getString("pass_key").equals("password matched")) {
                        ...
                        SharedPreferences.Editor sp = getSharedPreferences("Sanction_info", MODE_PRIVATE).edit();

                        sp.putString("sid" , sanction_id);

                        sp.commit();


                        Intent i = new Intent(SanctionLogin.this, SanctionHome.class);
                        startActivity(i);
                        finish();
